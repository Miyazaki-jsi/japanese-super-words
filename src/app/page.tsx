...
export default function Home() {
  ...
  const [showSituationSearchSuggestions, setShowSituationSearchSuggestions] = useState(false);
  const situationSearchRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (wordSearchRef.current && !wordSearchRef.current.contains(target)) {
        setShowSearchSuggestions(false);
      }
      if (situationSearchRef.current && !situationSearchRef.current.contains(target)) {
        setShowSituationSearchSuggestions(false);
      }
    };
  ...
  const allSearchableSituations = useMemo(() => {
    const free = freeSituations.map((sit) => ({ ...sit, isPremium: false as const }));
    const premium = premiumSituations.map((sit) => ({ ...sit, isPremium: true as const }));
    return [...free, ...premium];
  }, [freeSituations, premiumSituations]);

  const situationSearchResults = useMemo(() => {
    const q = situationSearchQuery.trim();
    if (!q) return [];
    return allSearchableSituations
      .filter((sit) => situationMatchesQuery(sit, q))
      .slice(0, 8);
  }, [situationSearchQuery, allSearchableSituations]);

  const openSituationFromSearch = (sit: {
    id: SituationId;
    isPremium: boolean;
  }) => {
    setSituationSearchQuery('');
    setShowSituationSearchSuggestions(false);
    if (!sit.isPremium || isPremiumUnlocked) {
            ...
            {!isStandaloneApp && showPwaHomeBanner && (
              <div className="relative flex items-stretch gap-1 animate-fade-in">
                <button
                  ...
                  className="btn-press flex-1 min-w-0 flex items-center gap-2 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-violet-50 px-3 py-1.5 text-left hover:border-indigo-200 hover:shadow-sm transition-all"
                >
                  <Smartphone className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0" />
                  <p className="min-w-0 flex-1 text-[11px] font-bold text-indigo-900 leading-snug truncate">
                    {jaOnly
                      ? 'ホーム画面に追加してみよう！'
                      : 'ホーム画面に追加してみよう！ · Add to Home Screen'}
                  </p>
                ...
                <button
                  ...
                  className="flex-shrink-0 w-8 rounded-xl border border-slate-100 bg-white text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-colors flex items-center justify-center"
              ...
              <div key="home-tab-situations" className="space-y-4 animate-fade-in">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-0.5 shrink-0">
                      {jaOnly ? 'ほかの無料' : 'More free · ほかの無料'}
                    </p>
                    <div ref={situationSearchRef} className="relative flex-1 min-w-0">
                      <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-400 pointer-events-none" />
                      <input
                        type="text"
                        value={situationSearchQuery}
                        onChange={(e) => {
                          setSituationSearchQuery(e.target.value);
                          setShowSituationSearchSuggestions(true);
                        }}
                        onFocus={() => {
                          if (situationSearchQuery.trim()) setShowSituationSearchSuggestions(true);
                        }}
                        placeholder={jaOnly ? 'シチュ検索' : 'Search scenes…'}
                        className="w-full pl-7 pr-7 py-1 text-[11px] font-semibold text-slate-800 bg-white border border-slate-100 rounded-lg shadow-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-200 transition-all"
                        aria-label={jaOnly ? 'シチュエーション検索' : 'Search situations'}
                      />
                      {situationSearchQuery && (
                        <button
                          type="button"
                          onClick={() => {
                            setSituationSearchQuery('');
                          ...
                          aria-label="Clear situation search"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      )}

                      {showSituationSearchSuggestions && situationSearchQuery.trim() && (
                        <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-slate-100 rounded-xl shadow-lg overflow-hidden max-h-56 overflow-y-auto">
                          {situationSearchResults.length > 0 ? (
                            situationSearchResults.map((sit) => {
                              const Icon = sit.icon;
                              const locked = sit.isPremium && !isPremiumUnlocked;
                              return (
                                <button
                                  key={sit.id}
                                  type="button"
                                  onClick={() => openSituationFromSearch(sit)}
                                ...
                                >
                                  ...
                                  {locked ? (
                                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-amber-50 border border-amber-100 flex-shrink-0">
                                      <Lock className="w-2.5 h-2.5 text-amber-500" />
                                      <span className="text-[8px] font-bold text-amber-700">Pro</span>
                                    </span>
                                  ) : (
